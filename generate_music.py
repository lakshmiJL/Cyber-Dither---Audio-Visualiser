...
# Reduce duration to 15 seconds to make deployment easier/faster
...
def generate_cyber_pop(bpm=124, duration=15):
...
def generate_dance_energy(bpm=130, duration=15):
    num_samples = duration * 44100
    samples_per_beat = int((60/bpm) * 44100)
    samples = []
    for i in range(num_samples):
...
def generate_lofi_pop(bpm=90, duration=15):
    num_samples = duration * 44100
    samples_per_beat = int((60/bpm) * 44100)
    samples = []
    for i in range(num_samples):
...
if __name__ == "__main__":
    ...
    write_wav("public/audio/generated-track.wav", generate_dance_energy())
